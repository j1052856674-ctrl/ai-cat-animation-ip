...
reads:
  - file: content/账号策略.md
    purpose: 人设和语气规范
  - file: skills/content-ops/schemas/script-outline.schema.json
    purpose: 产出格式契约
---
...
短视频的核心竞争力在脚本结构：前 3 秒决定完播率，中间节奏决定留存率，结尾互动决定转化和涨粉。本模块把选题卡转化为可拍摄的结构化脚本大纲——不写逐字稿（限制创作者发挥），而是设计关键的骨架节点，让创作者知道每一段拍什么、怎么讲、为什么这样安排。

## Inputs

| 字段 | 必填 | 说明 |
|------|:--:|------|
| `topic_card` | 条件必填 | 创作者选定的选题卡（来自 `02-topic` 输出）；如果缺失但用户提供了足够信息，可基于用户输入生成临时选题卡 |
...
**降级路径（选题卡缺失）：**
不直接卡死。执行以下降级：
1. 提示创作者："当前缺少选题卡。建议先执行选题推荐（Module 02）。"
2. 如果创作者提供了话题/方向/大致想法 → 基于用户输入**生成临时选题卡**（`id: temp-<timestamp>`）。
3. 如果创作者无任何输入 → 输出 `status: "blocked"`，引导至 Module 02。
...
| 缺失项 | 处理 |
|------|------|
| 无选题卡 + 用户无输入 | **不卡死**；输出 `status: "blocked"`，`blocking_reason: "缺少选题卡"`，`next_step: "02-topic"` |
...
```yaml
skill_run_id: "content-ops-module-03-{YYYYMMDD}-{NNN}"
...
7. **缺选题卡不卡死**：引导至 Module 02 或基于用户输入生成临时选题卡；不陷入无输入死循环。
