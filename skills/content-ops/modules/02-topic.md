...
depends_on:
  - module: module-00
    purpose: 竞品样本作为选题方向依据
reads:
  - file: content/账号策略.md
    purpose: 内容支柱和定位
  - file: content/竞品调研数据库.md
    purpose: 方向分类参考
  - file: skills/content-ops/schemas/topic-card.schema.json
    purpose: 产出格式契约
---
...
内容创作者常有"不知道下一条拍什么"的困境。本模块基于账号策略、粉丝画像、爆款拆解经验与历史数据，生成 5-10 条选题建议，让创作者的每条作品都有明确的选题依据而非凭直觉，提高爆款概率，避免选题同质化。
...
> "当前无近期竞品采集数据，以下选题基于策略、画像和爆款规律生成，属于**待验证假设**。建议先执行竞品数据采集（Module 00）以降低试错成本。"
...
```yaml
skill_run_id: "content-ops-module-02-{YYYYMMDD}-{NNN}"
...
next_step: "03-script"  # 创作者选定某条选题卡后
